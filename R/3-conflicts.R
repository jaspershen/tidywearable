#' Conflicts between the tidywearable and other packages
#'
#' This function lists all the conflicts between packages in the tidywearable
...
#' @export
#' @examples
#' tidywearable_conflicts()
tidywearable_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", tidywearable_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "tidywearable_conflicts")
}

tidywearable_conflict_message <- function(x) {
  if (length(x) == 0)
    return("")
  
  header <- cli::rule(left = crayon::bold("Conflicts"),
                      right = "tidywearable_conflicts()")
...
#' @export
print.tidywearable_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(tidywearable_conflict_message(x))
}
