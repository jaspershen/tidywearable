msg <- function(..., startup = FALSE) {
  if (startup) {
    if (!isTRUE(getOption("tidywearable.quiet"))) {
...
#' List all packages in the tidywearable
#'
#' @param include_self Include tidywearable in the list?
#' @export
#' @examples
#' tidywearable_packages()
tidywearable_packages <- function(include_self = TRUE) {
  raw <- utils::packageDescription("tidywearable")$Imports
...
check_tidywearable.org <-
  function(pkg) {
    x <-
      readLines(
        paste0(
          "https://www.tidywearable.org/tidywearable-packages/",
          pkg,
          "_Description.txt"
        )
      )
